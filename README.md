...
**A privacy-first, ultra-low-latency Windows desktop interview companion and mock simulation studio.**
...
**BackDoor AI** is an augmented reference utility that operates silently in the background during live meetings. It intercepts conversational audio and screen context, performs instant semantic extraction, and delivers structured talking points, code frameworks, and behavioral answers to a stealthy, webcam-aligned HUD overlay.

---

## 💾 Instant Download & Installation (Windows 10 / 11)

| Package | Format | Direct Download Link |
| :--- | :--- | :--- |
| 🚀 **BackDoor AI Setup Installer** | Windows NSIS Setup (`.exe`) | [**⬇️ Download BackDoor AI Setup.exe**](./BackDoor%20AI%20Setup.exe) |
...
To maintain a stealthy presence, all background command consoles for sidecar binaries are completely hidden. Their standard outputs are routed to a centralized logs directory:

*   **Logs Location**: `%LOCALAPPDATA%\com.backdoor.desktop\logs\`
...
To package the application into a standalone Windows installer (`.exe`) and place it at the repository root, run the build script from the backend folder:
